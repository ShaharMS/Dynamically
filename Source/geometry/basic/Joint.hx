...
class Joint extends DraggableSprite implements IDrawable {
	...
	function set_outlineColor(c:Color) {
		outlineColor = c;
		redraw();
		return outlineColor;
	}

	function set_fillColor(c:Color) {
		outlineColor = c;
		redraw();
		return fillColor;
	}
	...
	public function new(x:Float, y:Float, letter:String) {
		...
		onMoved[0] = (_, _, _, _) -> {
			for (c in Connection.all) c.redraw();
		}
		onDragged[0] = (_, _, _, _) -> {
	...
	public overload extern inline function connect(joint:Joint, ?connectionText:String):Joint {
		// Dont connect something twice
		for (c in connections.concat(joint.connections)) {
	...
	public overload extern inline function connect(...joints:Joint):Joint {
		for (joint in joints) {
			var doNothing = false;
			// Dont connect something twice
			for (c in connections.concat(joint.connections)) {
